...
from ms_bot.lib.messages import confirm_kb, SEND_BOLNICHNYI_KB
from setup.logger import CustomLogger

logger = CustomLogger.get_logger('bot')


class BolnichnyiDialog(ComponentDialog):
    def __init__(
            self,
            user_state: UserState,
            dialog_id: str = None
    ):
        super(BolnichnyiDialog, self).__init__(dialog_id or BolnichnyiDialog.__name__)
        ...
        self.initial_dialog_id = "BolnichnyiDialog"

    async def confirmation_step(self, step_context: WaterfallStepContext) -> DialogTurnResult:
        logger.debug('confirmation_step %s', BolnichnyiDialog.__name__)

        return await step_context.prompt(
            TextPrompt.__name__, PromptOptions(
                prompt=Activity(
                    channel_data=json.dumps(confirm_kb('больничного')),
    ...
    async def local_menu_step(self, step_context: WaterfallStepContext) -> DialogTurnResult:
        logger.debug('local_menu_step %s', BolnichnyiDialog.__name__)
        ...
        if found_choice == 'KEY_CALLBACK:yes':
            ...
            return await step_context.prompt(
                TextPrompt.__name__, PromptOptions(
                    prompt=Activity(
                        channel_data=json.dumps(SEND_BOLNICHNYI_KB),
                        type=ActivityTypes.message,
                    ),
                    retry_prompt=MessageFactory.text('Зробіть вибір, натиснувши на відповідну кнопку вище'),
                )
    ...
    async def last_step(self, step_context: WaterfallStepContext) -> DialogTurnResult:
            logger.debug('last_step %s', BolnichnyiDialog.__name__)

            found_choice = step_context.result

            if found_choice == 'KEY_CALLBACK:bolnichnyi3':
